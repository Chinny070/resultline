...
## Stage 2.4A — Agreement 0 preparation (read-only)

The deployed TEST-ONLY contract remains `0xbc0279B6BB9f558D0da0C2E7fAD1266FCc6f890C` on Studio Dev (chain `61997`). Before any Agreement 0 write, authoritative reads returned `get_agreement_count() = 0` and `get_withdrawable(0xaffe15eec45b68835cc9e5b4ab85dd5deae8e70b) = 0`.

The runner now provides a read-only preparation button and a separately gated create button. The proposed constitution uses `POST_EVENT_VERIFICATION` for the historical Oscars proposition (Anora / Best Picture), with exact source `www.oscars.org/oscars/ceremonies/2025`, a 0.001 GEN stake, and dynamically computed future close/not-before/deadline timestamps. No Agreement 0 has been created and no lifecycle call is chained. The browser source-integrity gate remains blob `2736b26d7377fc5354935d736f96cef2a81c5d1d`.
...
## Current integration deployment

Deployment `0x9345b67b619bce9ccdb4ceebcf35c18da6a7e56c60de2548c896eaabc93155ea` finalized with successful execution. The authoritative deployment recipient/contract address is `0x2d94Ff289500230b8A185AA50B0756297C963dDf`. `gen_getContractSchema` and `gen_getContractCode` pass; deployed code hashes to `39e2c19441154d9a566652262a3f43553ad50fe6`, contains `datetime.now(timezone.utc)`, and contains no `gl.vm.get_timestamp()`. Initial agreement count and Participant A withdrawable are both zero. The runner now targets this address and prepares Agreement 0 with fresh timestamps; no Agreement 0 transaction has been submitted.
